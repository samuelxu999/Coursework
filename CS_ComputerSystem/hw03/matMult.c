...
void setC_Prod(int ra,int carb, int cb) {
	...
	int col_a_rb;
	int col_b;
	for(row_a=0;row_a<ra;row_a++) {
		for(col_b=0;col_b<cb;col_b++) {			
			int tmp_value=0;			
			for(col_a_rb=0;col_a_rb<carb;col_a_rb++) {			
				tmp_value+=matA[row_a][col_a_rb]*matB[col_a_rb][col_b];
			}
			matC[row_a][col_b]=tmp_value;
		}
	}
