		...
		foreach wd_pool_a [array names Pr_array_A] {
			...
			if { $wd_sam==$wd_pool_a } {
				foreach wd_pool_b [array names Pr_array_B] {
					if { $wd_sam==$wd_pool_b } {
						#calucate P[A|S]
						#if { [expr $Pr_array_A($wd_sam)-$Pr_array_B($wd_sam)]==0 } {break}
						if { $type_sample=="A" } {
							set tmp [expr ($wd_v-$Pr_array_B($wd_sam))/($Pr_array_A($wd_sam)-$Pr_array_B($wd_sam))]
						}
						if { $type_sample=="B" } {
							set tmp [expr ($wd_v-$Pr_array_A($wd_sam))/($Pr_array_B($wd_sam)-$Pr_array_A($wd_sam))]
						}
